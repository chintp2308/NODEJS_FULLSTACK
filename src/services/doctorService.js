    ...
    try {
      if (!data.arrSchedule || !data.doctorId || !data.formattedDate) {
        ...
        let toCreate = _.differenceWith(schedule, existing, (a, b) => {
          return a.timeType === b.timeType && +a.date === +b.date; // thêm dấu + để có thể so sánh giữa số nguyên và chuỗi string
        });
    ...
    try {
      if (!inputId) {
        ...
        let data = await db.User.findOne({
          ...
          // Sử dụng eager loading để lấy thêm thông tin từ các bảng liên kết
          include: [
            {
              model: db.Markdown,
              //chọn các trường lấy
